...
///
/// Controls how [`ProfileReport::render`] serializes the execution profile.
...
///
/// Supports rendering execution profiles as text trees, JSON, DOT (Graphviz),
/// Mermaid diagrams, and HTML reports.
#[derive(Debug)]
...
impl ProfileReport {
    /// Create a new profile report from execution statistics.
    pub fn new(stats: ExecutionStats) -> Self {
        Self { stats }
    }

    /// Return a reference to the underlying execution statistics.
...
///
/// Produced by [`ProfileReport::flat_operators`] for tabular display or analysis.
#[derive(Debug, Clone)]
pub struct OperatorProfile {
    /// Operator name (e.g., `"HashJoin"`, `"TableScan[users]"`).
    pub name: String,
    ...
    pub rows_out: usize,
    /// Number of Arrow record batches processed.
    pub batches: usize,
...
///
/// Records query execution statistics over time, identifies slow queries,
/// and computes aggregate metrics (average latency, p95, etc.).
pub struct QueryProfiler {
    history: RwLock<Vec<QueryProfileEntry>>,
    /// Maximum number of entries to retain.
    max_history: usize,
    /// Queries exceeding this threshold (in ms) are flagged as slow.
    slow_query_threshold_ms: f64,
}

/// A single recorded query profile entry.
///
/// Captures SQL text, timing, memory usage, and whether the query was slow.
#[derive(Debug, Clone)]
pub struct QueryProfileEntry {
    ...
    /// Peak memory consumption in bytes during execution.
    pub peak_memory_bytes: usize,
    /// Unix epoch timestamp in milliseconds when the query completed.
...
impl QueryProfiler {
    /// Create a new profiler with the given history limit and slow-query threshold.
...
///
/// Generated by [`suggest_optimizations`] after inspecting operator statistics.
#[derive(Debug, Clone)]
pub struct OptimizationSuggestion {
    /// Human-readable description of the suggested optimization.
    pub description: String,
    /// Estimated impact level (`"High"`, `"Medium"`, or `"Low"`).
    pub impact: String,
    /// The condition in the profile that triggered this suggestion.
...
///
/// Walks the operator tree and emits suggestions for full table scans,
/// missing filters, expensive sorts, nested-loop joins, and high memory usage.
...
///
/// Operators consuming more than 20% of total execution time are reported.
pub struct BottleneckAnalyzer;

/// A detected performance bottleneck.
///
/// Contains the operator name, its share of total execution time, and a suggestion.
#[derive(Debug, Clone)]
pub struct Bottleneck {
    /// Name of the bottleneck operator.
    pub operator_name: String,
    /// Percentage of total query time consumed by this operator.
    pub percentage_of_total: f64,
    /// Wall-clock time in milliseconds.
    pub elapsed_ms: f64,
    /// Optimization suggestion for this bottleneck.
...
///
/// Converts an [`ExecutionStats`] tree into a flat list of [`OtelSpan`] records
/// suitable for export to observability backends.
...
///
/// Represents one operator's execution within a distributed trace.
#[derive(Debug, Clone)]
pub struct OtelSpan {
    /// Unique trace identifier shared by all spans in the query.
    pub trace_id: String,
    /// Unique identifier for this span.
    pub span_id: String,
    /// Parent span identifier (`None` for the root operator).
    pub parent_span_id: Option<String>,
    /// Operator name (e.g., `"HashJoin"`).
    pub operation_name: String,
    /// Start time in nanoseconds since the Unix epoch.
    pub start_time_ns: u64,
    /// Duration of this operator in nanoseconds.
    pub duration_ns: u64,
...
impl OpenTelemetryExporter {
    /// Create a new exporter with the given service name.
...
///
/// Accumulates counters and gauges for query activity and formats them
...
pub struct PrometheusMetrics {
    ...
    /// Total number of queries that resulted in errors.
...
///
/// Produces output compatible with Brendan Gregg's `flamegraph.pl` tool.
...
///
/// Useful for detecting optimizer mis-estimates via Q-error analysis.
#[derive(Debug, Clone)]
pub struct CardinalityAnnotation {
    /// Operator name in the query plan.
    pub operator: String,
    /// Row count predicted by the optimizer.
    pub estimated_rows: usize,
    /// Row count observed at runtime.
...
impl CardinalityAnnotation {
    /// Create a new annotation. Selectivity is computed as `actual / estimated`.
...
///
/// Tracks estimated vs actual row counts and computes aggregate Q-error.
...
impl CardinalityAnnotator {
    ...
    /// Add an annotation for an operator.
...
///
/// Records queries that exceed a configurable threshold and generates
/// rule-based suggestions (e.g., avoid `SELECT *`, add `LIMIT`).
pub struct SlowQueryAnalyzer {
    /// Minimum execution time (ms) for a query to be considered slow.
    threshold_ms: u64,
...
///
/// Contains the original SQL, execution time, and auto-generated recommendations.
#[derive(Debug, Clone)]
...
impl SlowQueryAnalyzer {
    /// Create a new analyzer with the given threshold and history limit.
...
///
/// Periodically records which operator is active and its memory usage,
/// then aggregates samples to estimate per-operator time.
...
///
/// Captures the active operator, timestamp, and memory usage at one point in time.
#[derive(Debug, Clone)]
pub struct ProfileSample {
    /// Name of the operator being profiled.
    pub operator: String,
...
impl SamplingProfiler {
    /// Create a new sampling profiler with the given sample rate and capacity.
    ...
    /// Return the total number of collected samples.
