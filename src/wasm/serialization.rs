        ...
        match array.data_type() {
            ...
            DataType::Boolean => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to BooleanArray")
                    })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::Int8 => {
                let arr = array.as_any().downcast_ref::<Int8Array>().ok_or_else(|| {
                    BlazeError::type_error("Failed to downcast array to Int8Array")
                })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::Int16 => {
                let arr = array.as_any().downcast_ref::<Int16Array>().ok_or_else(|| {
                    BlazeError::type_error("Failed to downcast array to Int16Array")
                })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::Int32 => {
                let arr = array.as_any().downcast_ref::<Int32Array>().ok_or_else(|| {
                    BlazeError::type_error("Failed to downcast array to Int32Array")
                })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::Int64 => {
                let arr = array.as_any().downcast_ref::<Int64Array>().ok_or_else(|| {
                    BlazeError::type_error("Failed to downcast array to Int64Array")
                })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::UInt8 => {
                let arr = array.as_any().downcast_ref::<UInt8Array>().ok_or_else(|| {
                    BlazeError::type_error("Failed to downcast array to UInt8Array")
                })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::UInt16 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to UInt16Array")
                    })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::UInt32 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to UInt32Array")
                    })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::UInt64 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to UInt64Array")
                    })?;
                Ok(arr.value(idx).to_string())
            }
            DataType::Float32 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to Float32Array")
                    })?;
            ...
            DataType::Float64 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to Float64Array")
                    })?;
                let val = arr.value(idx);
                if val.is_finite() {
                    Ok(val.to_string())
            ...
            DataType::Utf8 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to StringArray")
                    })?;
            ...
            DataType::LargeUtf8 => {
                let arr = array
                    ...
                    .ok_or_else(|| {
                        BlazeError::type_error("Failed to downcast array to LargeStringArray")
                    })?;
                Ok(format!("\"{}\"", escape_json_string(arr.value(idx))))
            }
